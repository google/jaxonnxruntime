  ...
  def _prepare(
    ...
    if config.jaxort_only_allow_initializers_as_static_args:
      if node.inputs[1] not in node.context_graph.get_constant_dict():
        raise ValueError(
            f'{node.inputs[1]} is not constant but used as `repeat` of Tile'
      ...
      node.attrs_dict['repeat'] = tuple(
          node.context_graph.get_constant_dict()[node.inputs[1]].tolist()
      )
    else:
      node.attrs_dict['repeat'] = tuple(inputs[1].tolist())
...
@functools.partial(jax.jit, static_argnames='repeat')
def onnx_tile(*input_args, repeat):
  """https://github.com/onnx/onnx/blob/v1.12.0/docs/Operators.md#Tile for more details."""
  inputs, _ = input_args
  return jnp.tile(inputs, repeat)
