...
def _asarray(proto):
    return jnp.asarray(onnx.numpy_helper.to_array(proto).reshape(tuple(proto.dims)))
...
class Constant(handler.Handler):
  ...
  def _prepare(cls, node: onnx_node.OnnxNode, inputs: Sequence[Any], onnx_jax_impl: Any):
    ...
    if 'value_string' in node.attrs:
      ...
      for item in _attr_to_dtype:
        if item in node.attrs:
          node.attrs_dict['value'] = jnp.array(node.attrs[item], dtype=_attr_to_dtype[item])
        matched = matched + 1

    assert matched == 1, f"Should only provide one of value attributes, but get {matched}"
